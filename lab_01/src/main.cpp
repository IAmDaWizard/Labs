...
int main() {
  ...
  while (true) {
    ...
    std::cout << "7. Fill a position of a matrix by a value.    8. Fill a "
                 "matrix by myself. \n\n";
    std::cout << "0. Exit. \n\n";
    ...
    if (n == 1) {
      ...
      if (a < 1 || b < 1 || a > matrices->rows[i - 1] ||
          b > matrices->rows[i - 1]) {
        std::cout << "There's no row with this number!\n";
        continue;
      }
      matrix_row_swap(matrices->array[i - 1], matrices->cols[i - 1], a - 1,
                      b - 1);
    } else if (n == 5) {
      std::size_t a{}, b{};
      ...
      if (a < 1 || b < 1 || a > matrices->cols[i - 1] ||
          b > matrices->cols[i - 1]) {
        std::cout << "There's no column with this number!\n";
        continue;
      }
      matrix_col_swap(matrices->array[i - 1], matrices->rows[i - 1], a - 1,
                      b - 1);
    } else if (n == 6) {
      int a{};
      ...
      if (row < 1 || row > matrices->rows[a - 1] || column < 1 ||
          column > matrices->cols[a - 1]) {
      ...
      self_completion(matrices->array[a - 1], matrices->rows[a - 1],
                      matrices->cols[a - 1]);
    } else
      std::cout << "Choose an operation with a correct sequence number\n";
  }
